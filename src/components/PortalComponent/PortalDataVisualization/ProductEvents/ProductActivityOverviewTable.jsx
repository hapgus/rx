...
export const ProductActivityOverviewTable = () => {
  const { isDataState } = useDataContext(); // Access the context data
  const tableData = isDataState.productDataFilteredByDate; // Use filtered data
...
console.log('filt', filteredEvents)
  ...
  let chartData = null;
  try {
    chartData = transformWithSchema(sortedData, 'productActivityOverview'); // Adjust schema as needed
  } catch (error) {
