...
export const LandingPageActivityOverviewTable = () => {
  const { isDataState } = useDataContext(); // Access the context data
  const tableData = isDataState.pageDataFilteredByDate; // Use filtered data
  ...
  let chartData = null;
  try {
    chartData = transformWithSchema(sortedData, 'pageActivityOverview'); // Transform with the correct schema
  } catch (error) {
